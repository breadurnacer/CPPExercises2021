...
cv::Mat convertBGRToGray(cv::Mat img) {
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            float grayIntensity = 0.3*blue + 0.59*green + 0.11*red;
        ...
        for (int i = 1; i < width-1; ++i) {
            ...
            for (int dj = -1; dj <= 1; ++dj) {
                for (int di = -1; di <= 1; ++di) {
                    ...
                    // добавляем его яркость в производную с учетом веса из ядра Собеля
                }
            }
...
cv::Mat convertDXYToGradientLength(cv::Mat img) {
    int width = img.cols;
    int height = img.rows;
    cv::Mat WBImg(height, width, CV_32FC1); // создаем одноканальную картинку состоящую из 32-битных вещественных чисел
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            cv::Vec2f dxy = img.at<cv::Vec2f>(j, i);
            ...
            WBImg.at<float>(j, i) = sqrt(x*x+y*y);
        }
    }
    return WBImg;
}
