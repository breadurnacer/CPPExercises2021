                                              ...
                                              double gaussianNoiseSigma)
{
    ...
    // это генератор случайных чисел (см. https://en.cppreference.com/w/cpp/numeric/random/mersenne_twister_engine )
...
void Line::plot(cv::Mat &img, double scale, cv::Scalar color)
{
    ...
    cv::line(img, cv::Point(1, getYFromX(1)), cv::Point(2, getYFromX(2)), color, 1);
}
